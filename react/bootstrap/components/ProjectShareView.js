...
export default class ProjectShareView extends Component {
  state = {
    moonboard: true
  };
  switcher(bool) {
  ...
  componentDidMount() {
    let { props } = this;
    let { location: { query }, dispatch, unPackSharedProject } = props;
    unPackSharedProject(query);
  }
  render() {
    let { props, switcher, state: { moonboard } } = this;
    let { props:{ projects: { project, isLoading, modal } } } = this;
        ...
        <div className="main">
          ...
          <div className="main-content">
            <div className="container">
              ...
              { moonboard ? <MoonBoardView {...props} /> : <ProjectShareGridView {...props} /> }
            </div>
          </div>
        </div>
      );       
